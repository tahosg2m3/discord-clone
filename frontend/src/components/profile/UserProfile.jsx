...
export default function UserProfile() {
  ...
  const { isPresenceReady } = useSocket();
      ...
      <div className="h-[64px] bg-[#111827] flex items-center px-3 shrink-0 justify-between relative z-50 border-t border-white/[0.06]">
        ...
        >
          <div className="relative shrink-0">
            ...
            <div className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 border-[3px] border-[#111827] rounded-full ${isPresenceReady ? 'bg-[#34d399]' : 'bg-[#64748b]'}`}></div>
          </div>

          <div className="flex flex-col min-w-0">
            <span className="text-[14px] font-semibold text-[#F2F3F5] truncate block leading-tight">{user.username}</span>
            <span className="text-[12px] text-[#94a3b8] truncate block leading-tight">{isPresenceReady ? 'Çevrimiçi' : 'Bağlanıyor...'}</span>
          </div>
        </div>

        {/* SAĞ KISIM: İkonlar */}
        <div className="flex items-center text-[#B5BAC1]">
          <button className="p-1.5 hover:bg-[#313338] hover:text-[#DBDEE1] rounded-md transition-colors">
